...
$q="SELECT * FROM `restaurant` WHERE `db_key`='knx_hot_$db_key' ";
$q=mysql_query($q);
$r=mysql_fetch_assoc($q);
mysql_select_db("knx_hot_$db_key");
...
while ($r2=mysql_fetch_array($q2)) {
  ...
  while ($r3=mysql_fetch_assoc($q3)) {
    $product['qty']+=$r3['qty'];
    $product['price']=number_format($r3['price'], 2, '.', '');
...
$product = '
<table border="0" cellpadding="1" cellspacing="0" style="font-size:8px;border-top: 0.1 solid black;border-bottom: 0.1 solid black;">';

foreach ($sale_order_line as $product_line) {
  $product.='<tr>
  <td width="4" align="center"></td>
  <td width="15" align="center">'.$product_line['qty'].'</td>
  <td width="115" align="left">'.$product_line['name'].'</td>
...
$pdf->Output($_SERVER['DOCUMENT_ROOT'] ."project/hotapp/$db_key/$sale_order_id".".pdf ", 'F');
